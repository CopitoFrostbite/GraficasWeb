...
class Player {
    constructor(id, gltf, characterName, controls, scene, terrainMesh, database) {
        this.id = id;
    ...
    updateDatabase() {
        const playerRef = ref(this.database, `rooms/roomID1/players/${this.id}`);
        update(playerRef, {
            ...
            tackleHitbox: {
                min: this.tackleHitbox.min,
                max: this.tackleHitbox.max
            }
        });
    }
}
